...
> ⚠️ **Preview URL이 배포마다 다르다 → 기존 익명 세션이 이어지지 않을 수 있다.** Vercel은 배포마다 다른 호스트(`ai-wanted-<해시>-jihyeon.vercel.app`)를 준다. 브라우저 쿠키(Supabase 익명 세션 포함)는 **origin(호스트) 단위**라서, 새 Preview URL에 접속하면 이전 Preview에서 만든 익명 세션이 없고 `/demo-init`이 **새 익명 계정**을 만든다. 따라서 (1) 이전 Preview에서 만든 개인 아이템·배치는 새 URL에서 **안 보이는 것이 정상**이고(DB에는 남아 있으나 다른 계정 소유), (2) 스모크는 **하나의 Preview URL에서 처음부터 끝까지** 진행해야 하며, (3) 배포를 새로 하면 익명 계정이 늘어난다(§3). Vercel 로그인(Deployment Protection) 쿠키도 호스트별이라 새 URL에서 로그인을 다시 요구할 수 있다.
>
> ⚠️ **"섬 화면" = `/checkin` 등교 체크인을 끝낸 뒤 나오는 섬 화면**이다(체크인 → 아이템 준비 → 섬). **`/island`가 아니다.** `/island`는 등/하교 흐름과 독립된 3D 확인용 화면이라 `/api/island`를 호출하지 않고 공용 15종·개인 아이템을 불러오지 않는다(2026-09-20 Preview 진단). 아래 2~5번의 섬 확인은 모두 체크인 흐름의 섬에서 한다. 참고: 아이템 생성 대기 중 폴링은 요청 하나를 최대 12초까지 기다리고 응답 사이에 1초 간격을 둔다(2026-09-20 수정 — 이전에는 1초마다 요청을 취소해 결과를 못 받았다).
1. **입장**: A로 Preview `/checkin` 접속 → 자동으로 `/demo-init`을 거쳐 체크인 화면이 뜬다(로그인 화면 없음). B도 동일. (Supabase Users에 익명 계정 2개 생성 확인)
2. **공용 섬 동일**: A·B 모두 `/checkin` 체크인 완료 후 나오는 섬 화면(`/island` 아님)에서 **공용 15종의 모양·위치가 같다**. 공용 아이템은 옮기거나 지울 수 없다.
3. **A 체크인 완료 → 아이템**(`/checkin` 흐름): A가 발화에 `A-마커`를 넣어 체크인을 끝내면 "아이템 생성 중…" 뒤 아이템이 나오고 섬에 놓을 수 있다. 새로고침해도 A의 아이템이 **그 자리에 남는다**(서버 저장).
...
## 7. Preview 브라우저 스모크 결과 (2026-09-20)
- **대상**: Vercel **Preview** `https://ai-wanted-47gjufrrt-jihyeon.vercel.app` (커밋 `599dcdc`, Target Preview, Deployment Protection 유지). 이전 Preview URL과 origin이 달라 익명 세션이 이어지지 않으므로(§4 안내) 이 URL에서 처음부터 진행했다.
- **경로**: 섬 확인은 **`/island`가 아니라 `/checkin` 체크인을 끝낸 뒤 나오는 섬 화면**에서 했다(§4 안내). `/island`는 `/api/island`를 부르지 않는 별도 화면이다.
- **방문자**: A = 한 브라우저(익명 세션 1), B = 별도 브라우저(익명 세션 2).

| # | 확인 항목 | 결과 |
|---|---|---|
| 1 | **A**: 섬 화면에 공용 15개가 표시되고 **이동할 수 없다**(`locked`) | 통과 |
| 2 | **A**: 체크인 후 개인 아이템 **‘풍선’ 생성 성공**(폴백 ‘선물 상자’가 아닌 생성 아이템, 폴링 수정 후 화면에 정상 도착) | 통과 |
| 3 | **A**: 개인 아이템 배치 후 `/api/island` 응답에서 **`locked:false`**, **`x`/`z` 좌표** 확인 | 통과 |
| 4 | **B**(별도 브라우저): `/api/island`가 **공용 15개만** 반환 | 통과 |
| 5 | **B**: **`locked:false` 항목 0개**, **A의 ‘풍선’ 0개** | 통과 |
| 6 | 실제 Preview에서의 **A/B 개인 아이템·배치 격리** | **통과** |

- 이로써 로컬·공유 DB에서 검증한 격리(§14-10)가 **실제 Vercel Preview 환경에서도** 동일하게 성립함을 확인했다: 공용 15종은 모든 방문자에게 동일하고 수정 불가, 개인 아이템·배치는 만든 방문자에게만 보인다.
- 이 스모크로 함께 확인된 것: 브라우저 Supabase 클라이언트의 정적 환경변수 참조 수정(`42c13c5`)으로 `/demo-init` 익명 로그인이 동작하고, 폴링 수정(`599dcdc`)으로 아이템 생성 결과가 화면에 도착한다.
- **추가 확인(위 표 이후 실제로 확인한 내용)**:

  | 확인 항목 (체크리스트 §4 해당 번호) | 결과 |
  |---|---|
  | **교사 화면 격리 (#6)**: **B의 교사 화면에 A의 신규 녹음(체크인 발화)이 보이지 않았고, 공용 데모 기록만 표시됨** | **통과**(격리 확인 + 공용 대조군 표시) |
  | **개발용 경로 (#9의 일부)**: Preview에서 `/api/dev/login`이 **Not Found(404)** | **통과** |

- **아직 확인하지 않은 항목(정직한 기록)**:
  - **상담 신청 격리 (#7)**: 이번 수동 스모크에서 **미확인**.
  - **재입장 (#10, 쿠키 삭제 후 새 익명 계정·개인 아이템 비노출)**: 이번 수동 스모크에서 **미확인**.
  - **#9의 나머지 경로**(`/api/dev/reset-today`, `/api/dev/demo-island-preview` 404): 이번에 개별 확인한 기록이 없다(`/api/dev/login`만 확인).
  - 위 항목은 확인하면 이 절에 추가한다. 상담 신청 격리는 로컬 서버 + 공유 DB의 앱 A/B(§14-10)와 대시보드 격리 회귀 테스트(§16-3, `npm run test:dashboard-scope`)로는 이미 검증했지만, **Preview 브라우저에서는 아직 검증하지 않았다.**
- **`npm run test:demo-ab`는 이번 Preview에서 재실행하지 않았다.** 앱 A/B 자동 테스트(95/95)의 마지막 결과는 §14-10(로컬 서버 + 공유 DB)이다.
- 이 스모크가 공유 DB에 남긴 것: 익명 계정 2개(A·B)와 A의 체크인 세션·job·아이템 ‘풍선’·개인 섬·배치·자산(봉인 트리거로 지워지지 않는 잔재).
