...
- **범위 한정(정직한 기록)**: 체크리스트 §4의 10개 항목 중 위 표의 항목(공용 표시·이동 불가, 개인 아이템 생성·배치, `/api/island` 응답, A/B 격리)을 확인했다. 교사 화면 격리(#6)·상담 신청 격리(#7)·개발 경로 404(#9)·재입장(#10)은 이번 기록 범위에 포함하지 않았다(별도로 확인하면 이 절에 추가).
