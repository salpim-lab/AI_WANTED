...
- **AI 하루 요약(상세 화면·챗봇)**: 실제 세션의 요약은 DB(`analysis_runs`)에 저장·조회하도록 바뀌었다(**1095 적용 전까지는 공유 DB에 반영되지 않음**). 표시·생성 규칙: ① **본인 체크인이 있는 날짜에는 공용 요약 대신 본인 요약**을 쓴다(본인 요약이 없으면 "없음") ② **하교 기록이 있지만 통합(full) 요약이 없으면 챗봇은 색만** 쓴다 ③ **챗봇은 요약을 자동 생성하지 않고**, 상세 화면에서 생성·저장된 요약을 읽는다. 공용 세션과 방문자 세션을 한 요약에 섞지 않는다. 챗봇 컨텍스트 빌더(`context.ts`)는 대화 원문을 읽지 않는다 — 원문을 넣으려면 개인정보 정책 결정이 먼저다.
