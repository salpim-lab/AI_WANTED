...
| AI 하루 요약 DB 저장(**1095**) | ✅ **공유 DB 적용 완료**(2026-09-20, 원격 `20260920063259`) · ⬜ 앱 코드 미병합·미배포 | DB: 사전 14/14·사후 13/13·행동 검증 14/14(롤백)·기존 데이터 불변(fingerprint 동일) — 절차 문서 §17-10. 앱: 브랜치 `feature/jihyeon-session-summary-db`(main 미병합). 상세 화면과 협진 챗봇이 같은 저장 요약을 읽고, 새로고침·인스턴스 변경 후에도 유지된다. 로컬 테스트: `test:1095-local` 35/35, `test:session-summaries` 38/38. |
...
- **AI 하루 요약(상세 화면·챗봇)**: 실제 세션의 요약은 DB(`analysis_runs`)에 저장·조회하도록 바뀌었다(**DB는 1095 적용 완료, 앱 코드는 미병합·미배포**). 표시·생성 규칙: ① **본인 체크인이 있는 날짜에는 공용 요약 대신 본인 요약**을 쓴다(본인 요약이 없으면 "없음") ② **하교 기록이 있지만 통합(full) 요약이 없으면 챗봇은 색만** 쓴다 ③ **챗봇은 요약을 자동 생성하지 않고**, 상세 화면에서 생성·저장된 요약을 읽는다. 공용 세션과 방문자 세션을 한 요약에 섞지 않는다. 챗봇 컨텍스트 빌더(`context.ts`)는 대화 원문을 읽지 않는다 — 원문을 넣으려면 개인정보 정책 결정이 먼저다.
...
- [ ] **AI 하루 요약 DB 저장(1095) 앱 배포·확인** — DB 적용은 완료(2026-09-20). 남은 것: 앱 코드 main 병합(별도 승인) → Preview 배포 → 실제 AI로 1회 생성·새로고침·챗봇 확인, A 요약이 B에 안 보임, 재배포 후에도 유지. 절차 문서 §17-7·§17-10.
...
> **DB 1094·공용 시드·1095는 이미 적용됐으므로 이 순서에 DB 적용·재적용 단계는 없다.** 1095(AI 요약 DB 저장)의 앱 코드(`feature/jihyeon-session-summary-db`)를 배포하려면 main 병합 후 7번(Preview 배포)에서 함께 나가며, 배포 뒤 확인은 절차 문서 §17-7의 8~9번.
...
| **1095(AI 요약 DB 저장) 앱 배포·Preview 확인** | ⬜ 미수행 (DB 적용·검증은 2026-09-20 완료 — 절차 문서 §17-10) |
