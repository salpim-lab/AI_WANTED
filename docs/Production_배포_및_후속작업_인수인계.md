...
| AI 하루 요약 DB 저장(**1095**) | ✅ **공유 DB 적용 완료**(2026-09-20, 원격 `20260920063259`) · 🟡 **Preview 수동 검증 통과(일부)** · ⬜ main 미병합 | DB: 사전 14/14·사후 13/13·행동 검증 14/14(롤백)·기존 데이터 불변 — 절차 문서 §17-10. Preview `ai-wanted-hulxm60ay-jihyeon.vercel.app`(커밋 `d2be404`) 수동: A 상세 화면의 등교 요약 표시·DB 저장·상세/챗봇 같은 행·생성 후 챗봇 반영·B 챗봇에 A 내용 없음(**요약·챗봇 A/B 격리 통과**) — §17-11. **미확인**: 새로고침 후 유지, 재배포 후 유지. 후속 수정(발화 없는 stopped 하교, 로컬 완료·**미배포**): §17-12. 로컬 테스트: `test:1095-local` 35/35, `test:session-summaries` 46/46. |
...
- **AI 하루 요약(상세 화면·챗봇)**: 실제 세션의 요약은 DB(`analysis_runs`)에 저장·조회하도록 바뀌었다(**DB는 1095 적용 완료, 앱 코드는 미병합·미배포**). 표시·생성 규칙: ① **본인 체크인이 있는 날짜에는 공용 요약 대신 본인 요약**을 쓴다(본인 요약이 없으면 "없음") ② **아이 발화가 있는 하교 기록이 있지만 통합(full) 요약이 없으면 챗봇은 색만** 쓴다(발화 없이 중단된 하교는 하교로 세지 않고 등교 요약을 쓴다 — 로컬 수정 완료·미배포, §17-12) ③ **챗봇은 요약을 자동 생성하지 않고**, 상세 화면에서 생성·저장된 요약을 읽는다. 공용 세션과 방문자 세션을 한 요약에 섞지 않는다. 챗봇 컨텍스트 빌더(`context.ts`)는 대화 원문을 읽지 않는다 — 원문을 넣으려면 개인정보 정책 결정이 먼저다.
...
| **1095(AI 요약 DB 저장) 앱 배포·Preview 확인** | 🟡 일부 확인 — Preview(`d2be404`)에서 요약 표시·DB 저장·상세/챗봇 조회 행 일치·생성 후 챗봇 반영·A/B 격리 통과(§17-11). ⬜ 미확인: **새로고침 후 유지, 재배포 후 유지**, main 병합, Production. 발화 없는 stopped 하교 수정(§17-12)은 로컬 완료·미배포 |
