...
| 항목 | 상태 | 근거·비고 |
|---|---|---|
| `main` 기준 커밋 | ✅ | **코드 기준 최종 병합 커밋 `9434544`**. 이후 main에는 **문서 커밋만** 추가됨(코드 변경 없음). 이 문서 작성 시점 main 최신은 `8bc01c9`이고, 이 문서의 커밋이 그 다음이다. 최신은 `git log -1 main`으로 확인. |
...
| AI 하루 요약 DB 저장(**1095**) | ⬜ 로컬 구현·검증 완료, **공유 DB 미적용·미배포** | 브랜치 `feature/jihyeon-session-summary-db`(main 미병합). 상세 화면과 협진 챗봇이 같은 저장 요약을 읽고, 새로고침·인스턴스 변경 후에도 유지된다. 로컬 테스트: `test:1095-local` 35/35, `test:session-summaries` 38/38. 적용 전·후 검증 SQL은 준비만 됨(미실행) — 절차 문서 §17-7·§17-9. |
...
- **1095(AI 하루 요약 DB 저장)는 아직 공유 DB에 적용하지 않았다.** 적용하려면 **DB 먼저, 앱 배포는 그 뒤**(이전 앱은 요약을 DB에 쓰지 않으므로 1095가 먼저 있어도 안전. 새 앱을 1095 없는 DB에 배포하면 저장은 되지만 중복·혼합 방어가 DB에 없다). 적용 직전·직후 검증 SQL(`scripts/verify-1095/`)과 롤백(`supabase/rollbacks/1095_session_summary_guard_rollback.sql`, 데이터 손실 없음 — 단 롤백 시 앱 코드도 함께 되돌린다)은 절차 문서 §17에 있다. 신규 mock INSERT에는 예외가 없다(시드 요약의 *읽기* 호환만 정확한 조건으로 허용).
...
- **AI 하루 요약(상세 화면·챗봇)**: 실제 세션의 요약은 DB(`analysis_runs`)에 저장·조회하도록 바뀌었다(**1095 적용 전까지는 공유 DB에 반영되지 않음**). 방문자 본인 세션을 먼저 골라 그 안에서 등교/통합 요약을 정하고, 공용 세션과 방문자 세션을 한 요약에 섞지 않는다. 하교 세션이 있는 날은 통합 요약이 생기기 전까지 챗봇이 색만 쓴다. 챗봇 컨텍스트 빌더(`context.ts`)는 대화 원문을 읽지 않는다 — 원문을 넣으려면 개인정보 정책 결정이 먼저다.
- **범위 밖(후속)**: ① 시드(mock) 요약을 AI 요약과 구분 표기하지 않음 ② 공용·개인 요약 병기 없음 ③ 공용 세션 요약의 생성 권한·횟수 제한 없음 ④ **기간 요약(`consultation_period_summary`)은 여전히 서버 메모리**(인스턴스가 바뀌면 사라짐). 절차 문서 §17-8.
...
- [ ] **AI 하루 요약 DB 저장(1095) 적용 결정** — 적용은 별도 승인. 순서: 적용 직전 SQL → 1095 적용(MCP, `db push` 금지) → 적용 직후 SQL·행동 검증 → 앱 배포 → Preview에서 A/B 확인(A 요약이 B에 안 보임, 재배포 후에도 유지). 절차 문서 §17-7·§17-9.

### P2 (후속 개선)
- [ ] **섬 재방문 UI** (저장된 내 섬을 체크인 밖에서 다시 보는 화면).
- [ ] **공용 아이템 `note` 연출.**
- [ ] **`/island` 화면의 목적 정리 또는 제거.**
- [ ] **AI 요약 표시·생성 규칙(범위 밖)**: 시드(mock) 요약 구분 표기, 공용·개인 요약 병기 규칙, 공용 세션 요약의 생성 권한·횟수 제한, **기간 요약의 DB 저장**(`source_type='student'`, `demo_owner_id` 서버 채움 설계 필요).
- [ ] **테스트 잔재 정리 절차**(봉인 트리거 때문에 별도 승인·계획 필요, §2).
- [ ] **성능 개선과 Vercel/Supabase 리전 검토** — Supabase는 서울(`ap-northeast-2`, `docs/DATABASE_GUIDE.md`). **Vercel 함수 리전은 이 문서 작성 중 확인하지 않았다 ❓**(프로젝트 설정에서 확인). 함수와 DB 리전이 멀면 API 왕복이 누적돼 느려진다(폴링 문제에서 응답이 1초를 넘은 배경일 수 있음 — 추정).
...
> **DB 1094와 공용 시드는 이미 적용됐으므로 이 순서에 그 재적용 단계는 없다.** 단 **1095(AI 요약 DB 저장)는 미적용**이다 — 적용하기로 하면 3번(환경변수)~7번(Preview 배포) 사이, **앱 배포 전에** 1095를 적용한다(절차 문서 §17-7).
...
| `npm run test:session-summaries` | 요약 저장·조회·A/B 격리·재사용·멱등·인스턴스 변경(메모리 DB + **가짜 fetch**, 38개) | 없음 | **없음**(가짜 fetch) | 없음 | ✅ 가능 |
...
| **Vercel 함수 리전**, Vercel 플랜의 **함수 최대 실행 시간 한도** | ❓ 확인하지 못함(아이템 생성 라우트 `maxDuration=180`, 대화·전사 등 60. Supabase는 서울) |
| Supabase Auth **허용 도메인·URL** 설정 | ❓ 조회하지 않음 |
| 앱 **전체 AI 호출 한도**, 프로젝트 일일 총량·통합 금액 상한 | ⬜ 없음(P1) |
| **1095(AI 요약 DB 저장) 공유 DB 적용·앱 배포·Preview 확인** | ⬜ 미수행(로컬 검증만 — 검증 SQL도 준비만, 미실행) |
| AI 요약 **표시·생성 규칙**(시드 구분 표기·공용/개인 병기·공용 요약 생성 제한)과 **기간 요약 DB 저장** | ⬜ 범위 밖(후속) |
