...
| 담당 | 영역 |
|---|---|
| 이유민 | 학생 화면 — 섬 제외 전체 (교사 코멘트 확인, 색 선택, AI 대화(녹음 기반)) |
| 강윤지 | 학생 화면 — 3D 에셋·섬 배치, 상담 전문 기반 아이템 추론·생성 |
| 진승혜 | 교사 화면 — 대시보드 |
...
src/
  app/
    ...
    api/ai/
      ...
      item-extract/route.ts      # 강윤지 — 상담 전문 → 아이템 추론 (저장 없음, 확인용)
  ...
  lib/
    supabase/
      ...
      admin.ts                # API Route의 검증된 쓰기용(secret key, 서버 전용)
