...
const STYLE_HINTS = [
  ...
  "받아주는 말은 감탄이나 짧은 반응 한 마디(우와 / 아이고 / 저런 / 오 / 어머 등)로만 하고, 바로 질문한다.",
  "질문을 '~해 줄래?' '~들려줄래?' 처럼 부드러운 권유 모양으로 쓴다.",
  "받아주는 말과 질문을 각각 담백한 한 문장으로, 꾸밈 없이 쓴다.",
  "student_name 이 있으면 받아주는 말을 이름을 부르며 시작한다(\"민준아,\" / \"민준이는\"). 없으면 담백하게.",
  "받아주는 말에서 아이가 한 말 중 가장 마음에 남는 낱말 하나를 **그대로** 되짚고, 질문은 아주 짧게(15자 안팎) 쓴다.",
  "아이 이야기가 기쁘거나 힘든 일이면 그 마음에 함께하는 말로 조금 넉넉하게 받아준다(다만 두 문장·90자 안). 가벼운 일이면 부풀리지 말고 담백하게.",
] as const;
