...
const STYLE_HINTS = [
  ...
  "받아주는 말 첫 문장은 아이 말을 되짚고, 둘째 문장은 **아이가 말한 순간을 눈에 보이듯 그려** 마음을 알아준다. 아이가 말하지 않은 상황·사람·장면을 지어내지 않는다.",
  "받아주는 말은 감탄이나 짧은 반응 한 마디(우와 / 아이고 / 저런 / 오 / 어머 등)에 그 이야기의 핵심 낱말을 이어 쓰고, 바로 질문한다.",
  "질문을 '~해 줄래?' '~들려줄래?' 처럼 부드러운 권유 모양으로 쓰고, 그 앞 받아주는 말은 다정하게 한 문장.",
  "받아주는 말을 이야기의 결에 맞춘다: 기쁜 일이면 같이 들뜬 말, 힘든 일이면 목소리를 낮춘 짧고 묵직한 말. 질문은 담백하게.",
  "student_name 이 있으면 받아주는 말을 이름을 부르며 시작한다(\"민준아,\" / \"민준이는\"). 없으면 다정하게 담백하게.",
  "받아주는 말에서 아이가 한 말 중 가장 마음에 남는 낱말 하나를 **그대로** 되짚고, 그 낱말에 얽힌 마음을 곁에서 짐작하는 말('~했겠다')을 덧붙인다. 질문은 짧게.",
  "받아주는 말에 아주 짧은 비유 하나를 넣는다(예: '마음이 무거운 돌을 든 것 같았겠다'). 아이 마음을 단정하지 않고 억지로 넣지 않는다. 이야기가 가벼우면 비유 없이 다정하게.",
] as const;
