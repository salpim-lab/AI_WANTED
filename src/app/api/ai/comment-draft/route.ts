...
//
// 요청: POST { studentId, date }  →  200 { draft: string | null, sent: string | null }
//   sent: 그날 대화에 대해 교사가 이미 보낸 최종본 (보낸 적 없으면 null)
//   - 501: OPENAI_API_KEY 미설정 또는 테스트 대상 아님 → 화면이 mock 예시 초안을 쓴다
//   - 404: 담당 학급 밖 학생
// 초안 생성·저장은 lib/supabase/interpretation/teacherComment.ts.
// 교사 최종본 보내기는 students/actions.ts(sendTeacherCommentAction) — 초안은 제안일 뿐, 자동 발송·자동 저장 없음.
...
import { getOrCreateCommentDraft, getSentComment } from "@/lib/supabase/interpretation/teacherComment";
...
export async function POST(request: Request) {
  ...
  const sent = await getSentComment(input.student.studentId, input.sessions.map((s) => s.sessionId));
  ...
  if (onlyIds?.length && !onlyIds.includes(input.student.studentId)) {
    return NextResponse.json(
      { message: "테스트 대상 학생이 아닙니다 (DAILY_ANALYSIS_ONLY_STUDENT_IDS).", sent },
  ...
  try {
    ...
    return NextResponse.json({ draft, sent }, { headers: NO_STORE });
  } catch (error) {
    if (error instanceof DailyAnalysisUnavailableError) {
      return NextResponse.json({ message: error.message, sent }, { status: 501, headers: NO_STORE });
    }
