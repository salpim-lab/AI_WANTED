// 담당: 김현우
// 학부모 상담 리포트 "AI 분석 요약"의 기간 요약 — 날짜별로 이미 만든 AI 분석을 모아 AI가 한 번 더 요약한다.
// 새 대화 원문은 보내지 않는다: 입력은 날짜별 분석 문장 + 그날 등교·하교 색뿐이다.
...
export const PERIOD_SUMMARY_PROMPT_VERSION = "consultation-period-v1";
...
입력은 한 아이에 대해 날짜별로 이미 만들어 둔 AI 하루 요약과, 그날 아이가 등교·하교 때 스스로 고른 마음 색이다.
...
function buildUserMessage(input: PeriodSummaryInput, mask: (text: string) => string): string {
  ...
  return [`[기간] ${input.from} ~ ${input.to}`, "[날짜별 마음 색과 하루 요약 (오래된 날 → 최근 날)]", ...lines].join("\n");
}

function sourceKey(input: PeriodSummaryInput): string {
  return input.analyses.map((a) => a.analysisId).sort().join(",");
}
...
/**
 * 기간 요약 하나를 돌려준다. 날짜별 분석이 하나도 없으면 null (AI를 부르지 않는다).
...
export async function getOrCreatePeriodSummary(input: PeriodSummaryInput): Promise<PeriodSummary | null> {
  if (input.analyses.length === 0) return null;
