...
// 날짜(기본 오늘, 미래는 못 고름)를 고르면 예정된 상담은 그날부터 앞으로 잡힌 것 전부, 완료한 상담은 상담 일시가 그날인 것만 보여준다.
...
async function ConsultationPageInMockScope({ searchParams }: PageProps<"/consultation">) {
  ...
  // 달력은 오늘까지만 고를 수 있다 — 주소로 미래 날짜를 넣어도 오늘로 되돌린다
  ...
  const [allEntries, allScheduled] = await Promise.all([
    listConsultationLogs(teacher.classId, searching ? filter : { ...filter, date }, teacher.id),
  ...
  const scheduled = allScheduled.filter(
    (c) => toKstDate(c.scheduledAt) >= date && !isStudentSelfConsultation(c.counterpart),
  );
