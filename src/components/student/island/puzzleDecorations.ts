import { ISLAND_SCALE, seededRandom, type IslandLayout, type DecorationKind } from "./placement";
...
function makePuzzleDecorationPlan(layout: IslandLayout, pieceIndex: number, seed: number) {
  ...
  const visualW = W / ISLAND_SCALE;
  const edgeMargin = visualW * 0.08;
  const centreMargin = visualW * 0.208;
  const spacing = visualW * 0.005;
  ...
  const kinds: DecorationKind[] = ["tree", "tree", "tree", "pine", "pine", "pine", "pine", "rock", "rock", "rock", "flower", "flower", "flower", "flower", "bush", "bush", "bush", "grass", "grass", "grass"];
  for (let restart = 0; restart < 80; restart++) {
    ...
    for (let kindIndex = 0; kindIndex < kinds.length; kindIndex++) {
      ...
      const radius = (W / ISLAND_SCALE) * (kind === "tree" ? 0.105 : kind === "pine" ? 0.075 : kind === "rock" ? 0.03 : kind === "flower" ? 0.025 : 0.02);
      ...
      const footprint = kind === "tree" ? radius * 1.3 : kind === "rock" ? radius * 1.4
        : kind === "flower" ? radius * 0.5 + (W / ISLAND_SCALE) * 0.025 : kind === "bush" ? radius * 1.1 : radius * 0.6 + (W / ISLAND_SCALE) * 0.016;
  ...
  // Corner pieces can be narrower after the rounded board outline clips them.
  // A partial shore garden is preferable to squeezing decoration footprints
  // into the playable field (and keeps the 210-item area open).
