...
type IconName = "leaf" | "puzzle" | "sun" | "rotate" | "top" | "plus" | "minus" | "home" | "left" | "right" | "star";
...
export default function IslandExperience({ compact = false, studentName = "민준", incomingItem = null, baseItemCount = 0, placedGifts = NO_GIFTS, preparing = false, onComplete }: Props) {
  const acquired = {
  ...
  return <div lang="ko" className={`island-experience ${compact ? "island-compact flex min-h-0 flex-1 flex-col text-[#2f3560]" : "min-h-dvh bg-[#f7f8f2] text-[#294638] [font-family:'Apple_SD_Gothic_Neo','Malgun_Gothic',sans-serif]"} [&_button]:cursor-pointer`}>
    ...
    <main className={compact ? "flex min-h-0 flex-1 flex-col" : "mx-auto max-w-[1440px] px-2 pb-8 pt-9 sm:px-8 lg:px-12"}>
      ...
      <section className={`relative isolate overflow-hidden bg-[#dceee5] ${compact ? "min-h-0 flex-1" : "island-viewport-panel rounded-[26px] border border-[#d6e3d7]"}`} aria-label="3D 떠 있는 섬">
        ...
        {showPlacementCta && <div className={`island-guidance-card island-cta-enter ${entryStage === "exiting" ? "island-cta-exit" : ""}`} role="status" aria-live="polite">
          <div className="min-w-0 flex-1" role={preparing ? "status" : undefined} aria-live="polite"><p className="text-[15px] font-bold tracking-[-0.35px] break-keep">{preparing ? waitingTick >= 6 ? "생각보다 준비가 오래 걸리고 있어" : "네 이야기를 담은 선물 준비 중…" : `${acquired.emoji} ${acquired.name}`}</p><p className="text-[12px] text-[#7d849b] break-keep">{preparing ? WAITING_MESSAGES[waitingTick % WAITING_MESSAGES.length] : "오늘 받은 아이템을 섬에 놓아 볼까?"}</p></div>
          <button onClick={startPlacement} disabled={preparing || entryStage === "exiting"} className="h-[30px] shrink-0 rounded-full bg-[#5a52f0] px-2.5 text-[12px] font-semibold text-white disabled:opacity-40">{preparing ? "선물 준비 중" : "내 섬에 놓기"}</button>
        </div>}
        ...
        {phase !== "moving" && showSceneControls && <div className={`absolute inset-x-0 bottom-0 z-10 flex flex-col items-center gap-4 ${compact ? "pb-3 pt-6" : "pb-5 pt-10"}`}>
          <div className="flex items-center gap-1 rounded-full border border-white/80 bg-white/90 px-2 py-1.5 shadow-[0_4px_20px_rgba(60,68,110,0.1)] backdrop-blur-sm" role="group" aria-label="시점 조작">
            {([ ["left", "left", "왼쪽으로 회전"], ["right", "right", "오른쪽으로 회전"], ["out", "minus", "축소"], ["in", "plus", "확대"], ["top", "top", "위에서 보기"], ["home", "rotate", "처음 시점으로"] ] as [CameraPreset, IconName, string][]).map(([preset, icon, label], index) => <button key={preset} title={label} aria-label={label} onClick={() => camera(preset)} className={`${button} h-8 w-8 text-[#5b6478] hover:bg-[#f1f3f8] max-sm:w-7 ${index === 2 || index === 4 ? "ml-1 border-l border-[#dfe3ee]" : ""}`}><Icon name={icon} size={17}/></button>)}
          </div>
          ...
          {mode === "island" && phase === "choosing" && entryStage === "placement" && <button onClick={() => sceneRef.current?.toggleOverview()} className="absolute left-4 bottom-5 rounded-full bg-white/85 px-3 py-1.5 text-[11px] text-[#5b6478]">{overview ? "캐릭터 보기" : "전체 보기"}</button>}
        </div>}
      </section>
