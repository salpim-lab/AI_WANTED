...
export default function IslandExperience({ flow = "checkin", compact = false, studentName = "민준", incomingItem = null, baseItemCount = 0, plain = false, placedGifts = NO_GIFTS, preparing = false, onComplete }: Props) {
  const acquired = {
  ...
  function finishPuttingDown() {
    ...
    after(AFTER_PUT_DOWN_PAUSE_MS, () => {
      setGifts((current) => [...current, { ...placed, id: `gift-${++giftId.current}` }]);
      setPlacementNotice(null);
      setPhase("returning");
    });
  }
